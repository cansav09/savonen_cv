...
pubs <- scholar::get_publications("zcRmG_UAAAAJ&hl=en") %>%
  dplyr::select(-cites, -cid, -number) %>%
  dplyr::rename_all(toupper) %>% 
  dplyr::arrange(desc(YEAR))

pubs$AUTHOR <- gsub("CL Savonen|C Savonen", " **C Savonen** ", pubs$AUTHOR)
pubs$JOURNAL <- c(as.character(pubs$JOURNAL[1:6]), "Oakland Univ. Honors College Thesis")

pubs <- knitr::kable(pubs, "html") %>% 
  kableExtra::kable_styling() %>% 
  kableExtra::column_spec(3:5, "5em") 
